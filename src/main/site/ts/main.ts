...
function postAndUpdate(): void {
  ...
  //  Remember to add a type annotation for the response data using the Matches type you defined above!
    ...
    const result = fetch("http://localhost:4567/results", {
      method: 'POST',
      body: JSON.stringify(postParameters),
      headers: {
        "Access-Control-Allow-Origin":"*"
      }}).then((response) => response.json()).then(x => updateSuggestions(x.matches))
...
document.addEventListener("keyup", async function test () {updateValues("Aries", "Gemini", "Cancer")})
postAndUpdate()
